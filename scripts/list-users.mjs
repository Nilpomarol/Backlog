...
const result = await client.execute("SELECT email, name, role, CASE WHEN firebase_uid IS NULL THEN 'pending' ELSE 'linked' END AS status FROM users ORDER BY role, email");
