...
export const backlogItems = sqliteTable("backlog_items", {
  ...
  description: text("description"),
  type: text("type", { enum: ["bug", "feature", "improvement", "task", "investigation"] }).notNull(),
  ...
  effort: text("effort", { enum: ["small", "medium", "large", "unknown"] }).notNull().default("unknown"),
  ...
  check("backlog_items_type_check", sql`${table.type} in ('bug', 'feature', 'improvement', 'task', 'investigation')`),
